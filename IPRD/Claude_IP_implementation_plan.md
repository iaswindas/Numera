...
- Use `scipy.optimize.milp` as primary solver (already in dependency tree via scikit-learn)
- Optional `ortools` for production-grade B&B (add as optional dependency)
...
ng_milp_use_ortools: bool = False
...
scipy==1.14.*          # Already transitive via scikit-learn, pin explicitly
# ortools-python>=9.9  # Optional: uncomment for production B&B solver
```
...
class DocumentGCN(nn.Module):
    ...
    Architecture:
      Layer 1: GCNConv(input_dim, 128) + BatchNorm + ReLU
      Layer 2: GCNConv(128, 64) + BatchNorm + ReLU  
...
numera:
  audit:
    zk-rfa:
      enabled: false
      chameleon-key-provider: "local"  # "local", "aws-kms", "vault"
...
| ocr-service | `torch-geometric` | 2.6.* | GCN for STGH (IP-5) — *optional, can use pure PyTorch* |
...
> [!IMPORTANT]
> **Q1: GCN Framework for IP-5 (STGH)**
> Should we use `torch-geometric` (heavier, more features) or implement a minimal GCN using raw PyTorch (lighter, fewer dependencies)? The GCN is only 3 layers with basic message passing — pure PyTorch is sufficient but torch-geometric would be faster to implement.

> [!IMPORTANT]  
> **Q2: MILP Solver for IP-2 (NG-MILP)**
> `scipy.optimize.milp` is adequate for most financial statements (< 500 cells). Should we add Google `ortools` as an optional dependency for enterprise-scale documents, or is scipy sufficient for the pilot?

> [!WARNING]
> **Q3: Chameleon Hash Key Management (IP-3)**
> The trapdoor key for chameleon hashes must be stored securely. Options:
> - **Local file** (dev only, insecure)
> - **AWS KMS** (production, requires AWS account)
> - **HashiCorp Vault** (self-hosted, enterprise)
> Which key management approach should we target for the initial implementation?

> [!IMPORTANT]
> **Q4: Phase Priority**
> The plan proposes executing Phase 1 (IP-2 + IP-5) first as they have zero dependencies. Should we start with Phase 1, or is there a different IP you'd like prioritized?
...
- Confirm DB migrations apply cleanly on existing data
